...
class Ticket extends Component{
  ...
  getStopsStr(numOfStops){
    ...
    if(numOfStops===1)
      return '1 пересадка'
    if(numOfStops>1&&numOfStops<5)
      return numOfStops+' пересадки'
    if(numOfStops>=5)
      return numOfStops+' пересадок'
  }
  render(){
    const f = this.appController.classesToCssModulesFormat.bind(this.appController)
    let {origin:origin_there, destination:destination_there, date:date_there, stops:stops_there, duration:duration_there} = {origin:'MOW', destination:'HKT', date:'2022-10-08T06:32:30.353Z', stops:['HKG', 'JNB'], duration:614}
    let {origin:origin_back, destination:destination_back, date:date_back, stops:stops_back, duration:duration_back} = {origin:'HKT',destination:'MOW',date:'2023-04-21T02:42:31.235Z',duration:1168,stops:['IST','JNB','HKG']}
